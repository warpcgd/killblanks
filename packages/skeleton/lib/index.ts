...
async function traverse(options: ModType, element: HTMLElement = document.documentElement) {
  const {
    remove,
    excludes,
    text,
  ...
  ;(function preTraverse(ele: HTMLElement) {
    ...
    // skip base64
    if (
      (BASE64.test(styles.backgroundImage) || isBase64Img(ele as HTMLImageElement)) &&
      skipBase64
    ) {
      return true
    }
    ...
    if (ele.tagName === 'IMG' || isBase64Img(ele as HTMLImageElement)) {
  ...
  await Promise.all(
    hasImageBackEles.map(e => {
      return handler.background(e, image)
    })
  )
  await Promise.all(
    imgs.map(e => {
      return handler.image(e as HTMLImageElement, image)
    })
  )
  pseudos.forEach(e => handler.pseudos(e, pseudo))
  await Promise.all(
    gradientBackEles.map(e => {
      return handler.background(e, image)
    })
  )
  ...
  if (offScreenParagraph && offScreenParagraph.parentNode) {
    toRemove.push(offScreenParagraph.parentNode as HTMLElement)
  }
  toRemove.forEach(e => removeElement(e as HTMLElement))
  // animation
  if (animation) {
...
async function genSkeleton(options: ModType = AUTOMATICMOD) {
  const { hide } = options
  // 将 `hide` 队列中的元素通过调节透明度为 0 来进行隐藏
  if (hide.length) {
    const hideEle = $$(hide.join(','))
    Array.from(hideEle).forEach(ele => setOpacity(ele as HTMLElement))
  }
  /**
   * walk in process
   */
...
async function outputSkeleton(element: HTMLElement, options: ModType | string = DEFAULTMOD) {
  if (options === undefined || options === null) {
    options = AUTOMATICMOD
  }
  if (typeof options === 'object') {
    options = merge(DEFAULTMOD, options as object)
  }
  clearCache()
  const { hide } = options as ModType
  // 将 `hide` 队列中的元素通过调节透明度为 0 来进行隐藏
  if (hide.length) {
    const hideEle = $$(hide.join(','))
    Array.from(hideEle).forEach(ele => setOpacity(ele as HTMLElement))
  ...
  const html = await traverse(options as ModType, element)
  /**
   * add `<style>`
   */
  let rules = ''
